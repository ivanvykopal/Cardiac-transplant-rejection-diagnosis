...
from tensorflow.keras.layers import Dense, Add, Multiply, concatenate, Input
...
class UnetBlock():
    ...
    def create_model(self, image_shape, name):
        inputs = Input(shape=image_shape)
        f1, p1 = self._downsample_block(
            inputs, self.config['filters'], self.num_conv)
        # 2 - downsample
        f2, p2 = self._downsample_block(
            p1, self.config['filters'] * 2, self.num_conv)
        # 3 - downsample
        f3, p3 = self._downsample_block(
            p2, self.config['filters'] * 4, self.num_conv)
        # 4 - bottleneck
        bottleneck = self._conv_block(
            p3, self.config['filters'] * 8, self.num_conv)

        # decoder:
        # 5 - upsample
        u5 = self._upsample_block(
            bottleneck, f3, self.config['filters'] * 4, self.num_conv)
        # 6 - upsample
        u6 = self._upsample_block(
            u5, f2, self.config['filters'] * 2, self.num_conv)
        # 7 - upsample
        u7 = self._upsample_block(
            u6, f1, self.config['filters'], self.num_conv)

        model = Model(inputs, u7, name=name)
        return model
